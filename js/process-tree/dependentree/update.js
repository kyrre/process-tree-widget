...
export function _update(source) {
  ...
  nodeEnter
    ...
    .style('stroke', d => {
      ...
      if (d.data.ImputedCreationTime) {
        return imputedNodeColor;
      }
      return circleStrokeColor;
    })
    .style('stroke-width', `${circleStrokeWidth}px`)
    .style('stroke-dasharray', d => d.data.Synthetic ? syntheticNodeStrokeDash : null)
    .style('fill', d => {
      if (d.data['Automated Note']) {
        if (d.data._maxDepth) {
          return maxDepthNodeColor;
